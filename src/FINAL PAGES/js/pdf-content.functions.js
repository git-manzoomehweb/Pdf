...
function fixSelfClosingTags(text) {
    ...
    // الگو: <tag> یا <tag > → تبدیل به <tag />
    selfClosingTags.forEach(tag => {
        const regex = new RegExp(`<${tag}(\\s[^>]*)?>`, 'gi');
        text = text.replace(regex, (match) => {
            // اگر قبلاً به صورت self-closing نباشه، اصلاح کن
            if (!match.endsWith('/>')) {
...
function cleanHtmlText(rawHtml) {
    const htmlFixed = fixSelfClosingTags(rawHtml);
    const rtlFixed = fixRTLTextCompletely(htmlFixed);
    return rtlFixed;
}
